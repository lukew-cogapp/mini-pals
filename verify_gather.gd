...
func _init() -> void:
	...
	inv = get_root().get_node("/root/Inventory")
	var tuning = get_root().get_node("/root/Tuning")
	...
	check("inventory starts empty", inv.count("wood") == 0
		and inv.count("stone") == 0 and inv.count("sphere") == 0)
	...
	await physics_frame
	player._punch()
	check("punching tree yields 1 wood", inv.count("wood") == 1)

	player._punch()
	player._punch()
	check("3 hits deplete tree (hidden, unavailable)",
		inv.count("wood") == 3 and not tree.visible and not tree.is_available())

	player._punch()
	check("punching a depleted node yields nothing", inv.count("wood") == 3)

	# --- Gathering: rock ---
	rock.set_deferred("global_position", player.global_position + Vector3(0, 0, -1.6))
	await physics_frame
	player._punch()
	check("punching rock yields 1 stone", inv.count("stone") == 1)
	...
	check("punch with nothing in reach yields nothing",
		inv.count("wood") == 3 and inv.count("stone") == 1)
	...
	check("craft with 3 wood 1 stone gives 1 sphere, costs 1+1",
		inv.count("sphere") == 1 and inv.count("wood") == 2
		and inv.count("stone") == 0)

	menu._craft()
	check("craft refused with 0 stone", inv.count("sphere") == 1
		and inv.count("wood") == 2)
	menu._refresh()
	...
	check("throw with 1 sphere spawns it and decrements",
		_sphere_count(w) == spheres_before + 1 and inv.count("sphere") == 0)
	player._throw_sphere()
